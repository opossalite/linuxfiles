...
match subcommand:
    case "new":
        ...
        name = sys.argv[2]
        contents = base.ezrun("ls -a ~/.pyv").splitlines()[2:]

        if name in contents:
            print(f"ERROR: Virtual environment named \"{name}\" already exists!")
            exit()

        base.ezrun("cd ~/.pyv && python3 -m venv " + name)
    ...
    case "rm" | "remove":
        ...
        result = base.ezrun("cd ~/.pyv && trash " + name)
    ...
    case "mv" | "rename" | "move":
        ...
        base.ezrun("cd ~/.pyv && mv " + name_old + " " + name_new)

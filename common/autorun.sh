...
if [[ $num_monitors == "1" ]]; then # ONE MONITOR
    ...
    case $HOSTNAME in
        ("VulpesKrovPC")
            #xrandr --output DP-0 --primary --mode 1920x1080 --rate 165 --output HDMI-0 --mode 1280x1024 --rate 75
...
if [[ $1 == "-r" || $1 == "--reload" ]]; then #run every time the script is run
    echo "Reloading"
    # don't do anything special for reloading, the rest of the file is for that
else #run only once at startup
    echo "Starting"
    ...
    #klipper &
    #flameshot &
    #xhost + & #this is for distrobox things, if you have that
    #lxpolkit &
